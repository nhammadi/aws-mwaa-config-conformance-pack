...
def evaluate_compliance(mwaa_client: boto3.client, timestamp: str) -> list:
    ...
    for env_name in env_list:
        ...
        web_server_mode = env_info.get("Environment", {}).get("WebserverAccessMode")
        if web_server_mode == "PRIVATE_ONLY":
            compliance_type = "COMPLIANT"
            annotation = (
                f"Public access for webserver in MWAA environment: '{name}' is enabled"
            )
        else:
            compliance_type = "NON_COMPLIANT"
            annotation = f"Public access for webserver in MWAA environment: {name} is not enabled"
