...
def evaluate_compliance(mwaa_client: boto3.client, timestamp: str) -> list:
    ...
    for env_name in env_list:
        ...
        endpoint = env_info.get("Environment", {}).get("EndpointManagement")
        if endpoint == "CUSTOMER":
            compliance_type = "COMPLIANT"
            annotation = f"VPC endpoints configured for MWAA environment: {name} is customer managed"
        else:
            compliance_type = "NON_COMPLIANT"
            annotation = f"VPC endpoints configured for MWAA environment: {name} is not customer managed"
        evaluations.append(
            build_evaluation(
